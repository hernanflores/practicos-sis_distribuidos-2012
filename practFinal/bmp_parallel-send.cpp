...
BYTE *process(BYTE *img1, BYTE *img2, int width, int chunksize){
  ...
  #pragma omp parallel for private(i,j,r1,g1,b1,r2,g2,b2,rf,gf,bf) shared(chunksize,width,img1,img2)
...
int main(int argc, char *argv[]){
  ...
  int procs, rank, chunksize, width, height, offset, chunkbytes, dimension;
  ...
  if(rank == MASTER){
    ...
    imgA = loadImg(argv[2], &headerA);
    imgB = loadImg(argv[3], &headerB);

    //valido la cantidad de dimensiones/procesos
    if(dimension == 1){
        if(procs != 2 && procs != 4 && procs != 6 && procs != 8){
          printf("FAIL: cant. de procs invalida para 1 dimension\n");
          return -1;
        }
        width = headerA.width;
        ...
        if(procs != 4 && procs != 16){
          printf("FAIL: cant. de procs invalida para 2 dimensiones\n");
          return -1;
        }
        width = headerA.width / (procs == 4 ? 2 : 4);
        height = headerA.height / (procs == 4 ? 2 : 4);
        chunksize = headerA.height / (procs == 4 ? 2 : 4); 
    }
  }
